...
if(!empty($subscriptions)) {
		...
		<table id="mepr-account-subscriptions-table" class="mepr-account-table">
		  <thead>
			<tr>
			  ...
			  <th><?php _ex('Subscription', 'ui', 'press-pool'); ?></th>
			  <th><?php _ex('Active', 'ui', 'press-pool'); ?></th>
			  <th><?php _ex('Created', 'ui', 'press-pool'); ?></th>
			  <th><?php _ex('Expires', 'ui', 'press-pool'); ?></th>
			  <th><?php _ex('Card Exp.', 'ui', 'press-pool'); ?></th>
			  <th> </th>
		  ...
		  <tbody>
			...
			foreach($subscriptions as $s):
			  if(trim($s->sub_type) == 'transaction') {
				...
				$prd      = $txn->product();
				$group    = $prd->group();
				$default  = _x('Never', 'ui', 'press-pool');
				...
				if($txn == false || !($txn instanceof MeprTransaction) || $txn->id <= 0) {
				  $default = _x('Unknown', 'ui', 'press-pool');
				}
			...
			?>
			  <tr id="mepr-subscription-row-<?php echo $s->id; ?>" class="mepr-subscription-row <?php echo (isset($alt) && !$alt)?'mepr-alt-row':''; ?>">
				<td data-label="<?php _ex('Membership', 'ui', 'press-pool'); ?>">
				...
				<td data-label="<?php _ex('Terms', 'ui', 'press-pool'); ?>">
					...
					<?php
					  if($txn != false && $txn instanceof MeprTransaction && $txn->is_sub_account()) {
						...
						<div class="mepr-account-sub-account-auto-rebill">
						  <?php _ex('Sub Account', 'ui', 'press-pool'); ?>
						...
						if($is_sub):
						  echo ($s->status == MeprSubscription::$active_str)?_x('Enabled', 'ui', 'press-pool'):MeprAppHelper::human_readable_status($s->status, 'subscription');
						elseif(is_null($s->expires_at) or $s->expires_at == MeprUtils::db_lifetime()):
						  _ex('Lifetime', 'ui', 'press-pool');
						else:
						  _ex('None', 'ui', 'press-pool');
						endif;
					  }
					?>
				  </div>
						  ...
						  if($prd->register_price_action == 'custom' && !empty($prd->register_price)) {
							...
							if($is_sub) {
							  ...
							  if($subscr->coupon_id && ($coupon = new MeprCoupon($subscr->coupon_id)) && isset($coupon->ID) && $coupon->ID) {
								$coupon_str = ' ' . _x('with coupon', 'ui', 'press-pool') . ' ' . $coupon->post_title;
							  }
							}
				  ...
				  <?php if($txn != false && $txn instanceof MeprTransaction && !$txn->is_sub_account && $is_sub && ($nba = $sub->next_billing_at)): ?>
					<div class="mepr-account-rebill"><?php printf(_x('Next Billing: %s', 'ui', 'press-pool'), MeprAppHelper::format_date($nba)); ?></div>
				  <?php endif; ?>
				</td>
				<td data-label="<?php _ex('Active', 'ui', 'press-pool'); ?>"><div class="mepr-account-active"><?php echo $s->active; ?></div></td>
				<td data-label="<?php _ex('Created', 'ui', 'press-pool'); ?>">
				  <?php if($txn != false && $txn instanceof MeprTransaction && $txn->is_sub_account()): ?>
					<div>--</div>
				  <?php else: ?>
				...
				<td data-label="<?php _ex('Expires', 'ui', 'press-pool'); ?>">
				  <?php if($txn != false && $txn instanceof MeprTransaction && $txn->is_sub_account()): ?>
					<div>--</div>
				  <?php else: ?>
					<div class="mepr-account-expires-at">
					  <?php if($txn != false && $txn instanceof MeprTransaction && $txn->txn_type == MeprTransaction::$payment_str || ($is_sub && !$sub->in_grace_period())) {
							...
							elseif($txn != false && $txn instanceof MeprTransaction && $txn->txn_type == MeprTransaction::$fallback_str) {
							  _ex('Never', 'ui', 'press-pool');
							}
							else {
							  _ex('processing', 'ui', 'press-pool');
							}
					  ?>
					</div>
				  <?php endif; ?>
				</td>
				<td data-label="<?php _ex('Card Expires', 'ui', 'press-pool'); ?>">
				  <?php if($txn != false && $txn instanceof MeprTransaction && $txn->is_sub_account()): ?>
					<div>--</div>
				  <?php else: ?>
					<?php if( ($exp_mo = $sub->cc_exp_month) && ($exp_yr = $sub->cc_exp_year) ): ?>
					  <?php $cc_class = (($sub->cc_expiring_before_next_payment())?' mepr-inactive':''); ?>
					  <div class="mepr-account-cc-exp<?php echo $cc_class; ?>"><?php printf(_x('%1$02d-%2$d', 'ui', 'press-pool'), $exp_mo, $exp_yr); ?></div>
					<?php else: //Need a placeholder for responsive ?>
					  <div>&zwnj;</div>
					<?php endif; ?>
				  <?php endif; ?>
				</td>
				<td data-label="<?php _ex('Actions', 'ui', 'press-pool'); ?>">
					  ...
					  if($txn != false && $txn instanceof MeprTransaction && ($txn->is_sub_account() || $txn->txn_type == MeprTransaction::$fallback_str)) {
						...
						elseif(!$is_sub && !empty($prd->ID)) {
						  if($prd->is_renewable() && $prd->is_renewal()) {
							?>
							  <a href="<?php echo $prd->url(); ?>" class="mepr-account-row-action mepr-account-renew"><?php _ex('Renew', 'ui', 'press-pool'); ?></a>
							<?php
						  }
						  ...
						  if($prd->can_you_buy_me()) {
							if($group !== false && $txn !== false && $txn instanceof MeprTransaction) {
							  ...
							  if(!$sub_in_group && !$life_in_group) { //$prd is in group, but user has no other active subs in this group, so let's show the change plan option
								MeprAccountHelper::purchase_link($prd, _x('Re-Subscribe', 'ui', 'press-pool'));
								MeprAccountHelper::group_link($txn);
							  }
							}
							else {
  ...
  <?php
    ...
    _ex('You have no active subscriptions to display.', 'ui', 'press-pool');
  ?></div><?php
}
