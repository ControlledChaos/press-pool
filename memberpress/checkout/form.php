  ...
  <form class="mepr-signup-form mepr-form" method="post" action="<?php echo esc_url($_SERVER['REQUEST_URI']).'#mepr_jump'; ?>" novalidate>
    ...
    <?php if( ($product->register_price_action != 'hidden') && MeprHooks::apply_filters('mepr_checkout_show_terms',true) ): ?>
      <div class="mp-form-row mepr_bold mepr_price">
        <?php $price_label = ($product->is_one_time_payment() ? _x('Price:', 'ui', 'press-pool') : _x('Terms:', 'ui', 'press-pool')); ?>
		...
		<?php if((!MeprUtils::is_user_logged_in() ||
		  ...
		  <div class="mp-form-row mepr_first_name">
			<div class="mp-form-label">
			  <label><?php _ex('First Name:', 'ui', 'press-pool'); echo ($mepr_options->require_fname_lname)?'*':''; ?></label>
		  ...
		  <div class="mp-form-row mepr_last_name">
			<div class="mp-form-label">
			  <label><?php _ex('Last Name:', 'ui', 'press-pool'); echo ($mepr_options->require_fname_lname)?'*':''; ?></label>
		...
		<?php else: ?>
		  ...
		  <?php if(!$mepr_options->username_is_email): ?>
			<div class="mp-form-row mepr_username">
			  <div class="mp-form-label">
				<label><?php _ex('Username:*', 'ui', 'press-pool'); ?></label>
				<span class="cc-error"><?php _ex('Invalid Username', 'ui', 'press-pool'); ?></span>
			  </div>
		  ...
		  <div class="mp-form-row mepr_email">
			<div class="mp-form-label">
			  <label><?php _ex('Email:*', 'ui', 'press-pool'); ?></label>
		  ...
		  <?php if($mepr_options->disable_checkout_password_fields === false): ?>
			<div class="mp-form-row mepr_password">
			  <div class="mp-form-label">
				<label><?php _ex('Password:*', 'ui', 'press-pool'); ?></label>
				<span class="cc-error"><?php _ex('Invalid Password', 'ui', 'press-pool'); ?></span>
			  </div>
			...
			<div class="mp-form-row mepr_password_confirm">
			  <div class="mp-form-label">
				<label><?php _ex('Password Confirmation:*', 'ui', 'press-pool'); ?></label>
				<span class="cc-error"><?php _ex('Password Confirmation Doesn\'t Match', 'ui', 'press-pool'); ?></span>
			  </div>
		...
		<?php if($product->adjusted_price() > 0.00 || !empty($product->plan_code)): ?>
		  <?php if($mepr_options->coupon_field_enabled): ?>
			...
			<div class="mp-form-row mepr_coupon mepr_coupon_<?php echo $product->ID; ?> mepr-hidden">
			  <div class="mp-form-label">
				<label><?php _ex('Coupon Code:', 'ui', 'press-pool'); ?></label>
				<span class="mepr-coupon-loader mepr-hidden">
				  <img src="<?php echo includes_url('js/thickbox/loadingAnimation.gif'); ?>" width="100" height="10" />
				</span>
				<span class="cc-error"><?php _ex('Invalid Coupon', 'ui', 'press-pool'); ?></span>
			  </div>
		...
		<?php if($mepr_options->require_privacy_policy && $privacy_page_link = MeprAppHelper::privacy_policy_page_link()): ?>
		  <div class="mp-form-row">
			<label for="mepr_agree_to_privacy_policy" class="mepr-checkbox-field mepr-form-input">
			  ...
			  <?php echo preg_replace('/%(.*)%/', '<a href="' . $privacy_page_link . '" target="_blank">$1</a>', __($mepr_options->privacy_policy_title, 'press-pool')); ?>
			</label>
		  </div>
		<?php endif; ?>
